        ...
        <div class="modal-content">
            ...
            <form action="/dashboard/antrian/" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
                    @csrf
                    <div class="mb-3">
                        <label for="nama_antrian" class="form-label">Nama Antrian</label>
                        <input type="text" class="form-control @error('nama_antrian') is-invalid @enderror"
                            id="nama_antrian" name="nama_antrian">
                        @error('nama_antrian')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('batas_antrian') is-invalid @enderror"
                            id="batas_antrian" name="batas_antrian" min="1" max="100">
                        @error('batas_antrian')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
...
<!-- Otomatis Membuatkan Slug yang mengambil dari field nama_antrian -->
<script>
    const nama_antrian = document.querySelector('#nama_antrian');
    const slug = document.querySelector('#slug');

    nama_antrian.addEventListener('change', function() {
        fetch('/dashboard/antrian/checkSlug?nama_antrian=' + nama_antrian.value)
...
<script>
    $(document).ready(function() {
        $('#tambah-menu-antrian form').submit(function(event) {
            ...
            var form = $(this);
            var nama_antrian = form.find('#nama_antrian').val();
            ...
            if (!nama_antrian) {
                form.find('#nama_antrian').addClass('is-invalid');
                error = true;
            } else {
                form.find('#nama_antrian').removeClass('is-invalid');
            }
